...
import {useGetStockQuery, useUpdateStockMutation} from "@/redux/feature/stockAPI/stockAPI.js";
...
const UpdateStock = () => {
    ...
    const [updateStock]=useUpdateStockMutation(id)
    ...
    useEffect(() => {
        if(data?.data){
            const stockData = data?.data.find(item => item.id.toString() === id);
            if(stockData){
                setinputForm({
                    ...
                    paid_amount: stockData.paid_amount || '',
                    due_amount: stockData.due_amount || '',
