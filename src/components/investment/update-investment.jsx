...
import {useGetInvestmentQuery, useUpdateInvestmentMutation} from "@/redux/feature/investmentAPI/investmentAPI.js";
...
const UpdateInvestment = () => {
    ...
    const {id}=useParams();
    const [updateInvestment]=useUpdateInvestmentMutation(id)
    ...
    useEffect(() => {
        if(data?.data){
            const investmentData = data?.data.find(item => item.id.toString() === id);
            if(investmentData){
                setinputForm({
                    ...
                    deed_image: investmentData.deed_image || '',
                    status: investmentData.status || '',
                    payment_type_id: investmentData.payment_type_id || '',
                });
            }
        }
    }, [data, id]);
    ...
    const handleSubmit=async (e)=>{
        ...
        try {
            ...
            await updateInvestment({id,data}).unwrap()
            alert("investment update successfully.");
            setinputForm({
                name:'',
                amount:'',
                description:'',
