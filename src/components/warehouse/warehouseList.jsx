...
import {
    ...
    usePaginationHouseQuery,
    useSearchHouseQuery
} from "@/redux/feature/warehouseAPI/warehouseAPI.js";
...
const WarehouseList = () => {
    ...
    const {data:paginateData,refetch}=usePaginationHouseQuery(page)
    const {data:searchData}=useSearchHouseQuery(search)
    const [deleteWarehouse]=useDeleteWarehouseMutation()

    const warehouseData = search
        ? (searchData?.data || [])
        : (paginateData?.data || []);
    ...
    const HandleonDelete=async(id)=>{
        ...
        if(result.isConfirmed){
            try {
                ...
                await showSuccess("Successfully deleted warehouse")
                refetch()
            }catch(e){
        ...
        <div className="p-6">
            <div className="flex justify-between items-center mb-4">
                ...
                <Link to='/create-house'>
                    <Button className="bg-blue-500 text-white">Create House</Button>
                </Link>
            </div>
            ...
            <Table>
                ...
                >
                  {item.status}
                            ...
                            <TableCell>
                                <Link to={`/get-stock-warehouse/${item.id}`}>
